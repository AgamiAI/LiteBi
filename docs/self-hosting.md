...
| `AGAMI_GOVERNANCE_ENFORCED` | no (**default off**) | Whether the **semantic-model pass** runs on this server. **On** (`1`/`true`/`yes`/`on`), a query may only name tables and columns your model declares, `SELECT *` is refused, and a model whose declared engine disagrees with its credentials is refused rather than guessed at. **Off** (unset, or any value not in that list), none of that runs: a query may read anything the connecting role is granted, including columns you left out of the model, and may enumerate your schema through catalog relations such as `information_schema.tables`. Every answer and every audit row still states that the checks did not run, so nothing silently claims otherwise. **Unaffected either way:** the read-only guard, the dangerous-function guard, the statement timeout and the row cap, and your database role, which stays the primary boundary. It ships off so a server can be brought up before the model-aware checks have been validated against your data; turn it on once they have. Read per request, so flipping it takes effect on the next query with no redeploy. |
