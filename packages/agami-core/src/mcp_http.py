...
def _build_org_resolver() -> SingleTenantOrgResolver:
    ...
    resolved by `tools.resolved_org_id()` (AGAMI_ORG_ID env -> datasource.yaml -> "local") — the SAME
    ...
    # operator can see "org_id=local (default)" and realize datasource.yaml/AGAMI_ORG_ID isn't reaching the
    # server (see F14's documented residual risk).
    source = "env" if os.environ.get("AGAMI_ORG_ID", "").strip() else (
        "default" if org_id == "local" else "datasource.yaml"
    )
