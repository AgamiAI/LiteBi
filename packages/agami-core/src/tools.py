...
SERVER_INSTRUCTIONS = (
    ...
    "it is != 'approved', and any metric in `receipt.columns.items[].metric` (the entries whose "
    "`column` is null) whose review_state != 'approved' — joins/metrics they haven't signed off; "
    "never hide them. A join's review_state is null when nothing matched it to a declared "
    "relationship, which is 'not matched yet' and NOT 'not signed off' — that gap is on the "
    "section's own `undetermined` sentence. `receipt.joins.items` is one entry per join the "
    "STATEMENT wrote, each `{predicate, from_to, scope, status}` with status declared / undeclared "
    "/ undeclarable / undetermined. Don't refuse on an unreviewed metric — answer and warn. ALSO "
    "read "
