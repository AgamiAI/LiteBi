...
| `joins` | **one entry per join the STATEMENT wrote**, not per declared relationship: `predicate` (the ON as the parser read it, `null` when the join wrote none), `from_to` (the two endpoints as the statement named them), `scope` (the same query-scope label the `tables` entries carry), and `status` ∈ `declared` / `undeclared` / `undeclarable` / `undetermined`. `undeclared` means the join wrote no predicate for a declaration to match (an explicit `CROSS JOIN`); `undeclarable` means an endpoint is a name the statement bound for itself (a CTE, a derived table), which no relationship in the model can be about; `undetermined` means it has not been settled. The trust block — `name`, `cardinality`, `confidence`, `review_state`, `origin`, `signed_off_by`/`_role`/`_at`, `cross_schema`, `on` — is what a MATCHED declaration contributes, and every one of those keys is `null` until the join is matched to one. A `null` `review_state` therefore means "not matched yet", **not** "not signed off": do not raise an unreviewed-join warning off it |
