...
| `tables` | **one entry per REFERENCE, not per table** — a table read twice appears twice. `ref` (the name as the statement wrote it), `alias`, `qname` (what the model resolved it to), `declared`, `rows`, `rows_as_of`, `freshness`, `scope`, `filters`. An **undeclared** reference (a CTE name, say) has `declared: false` and null `qname`/`rows`/`rows_as_of`/`freshness`, because there is no model row for those to be about. `scope` is `main` or `cte:<name>`, each carrying a trailing 1-based `#<n>` when that scope is one of **two or more** arms of a set operation (`UNION` / `INTERSECT` / `EXCEPT`) — `main#1`, `main#2`, `cte:recent#2` — or `subquery`, which **never** takes an ordinal because it names no scope to have arms. A plain `SELECT` and a single-arm CTE body take no suffix either. It is the query scope the reference was written in, and the only thing that tells two entries for the same `ref` apart: a table read in two arms is otherwise two identical rows. The ordinal is the arm's position **in the SQL**, not this list's order, which is the parse walk's — so it can skip and go backwards down the list, and the largest one is not the arm count. Strip it from the RIGHT (`rsplit('#', 1)[0]`) if you need the bare scope. `filters` is one `{"expr", "status"}` per declared `default_filters` entry of that table, `status` ∈ `applied` / `omitted` / `undetermined`: whether THAT reference's own scope wrote that exact predicate. `[]` when the table declares none |
| `joins` | **one entry per join the STATEMENT wrote**, not per declared relationship: `predicate` (the condition the join wrote, as the parser read it — an ON, a `USING` column list, or `NATURAL`; `null` when it wrote none of its own), `from_to` (the two endpoints as the statement named them), `scope` (the same query-scope label the `tables` entries carry), and `status` ∈ `declared` / `undeclared` / `undeclarable` / `undetermined`. `declared` means the predicate the statement wrote matches a relationship the model declares (named on `name`) — matched on the unordered set of `(table, column)` pairs the ON joins on, so operand order does not decide it and an extra filtering conjunct (`AND o.ts > c.ts`) does not weaken it; the declaration itself has to reduce **whole**, so a declared `on:` carrying anything but column equalities matches nothing rather than matching its unrestricted half. `undeclared` means it matches none, or it wrote no predicate for one to match (an explicit `CROSS JOIN`) — and every declaration between its two tables was one this layer could read; where one could not be (an `on:` that will not parse, or one holding a `:param`), the join is `undetermined` instead, because the gap is ours and not the model's. `undeclarable` means an endpoint is a relation the statement bound for itself (a CTE, a derived table, a `VALUES` list), which no relationship in the model can be about; `undetermined` means the analysis could not read the join — which is **not** a claim that the model does not declare it. The trust block — `name`, `cardinality`, `confidence`, `review_state`, `origin`, `signed_off_by`/`_role`/`_at`, `cross_schema`, `on` — is what the MATCHED declaration contributes, and every one of those keys is `null` on every other status. A `null` `review_state` therefore means "not matched", **not** "not signed off": do not raise an unreviewed-join warning off it |
| `aggregates` | **one entry per aggregate the statement computes**, each `{aggregate, scope, status, joins, findings}`, `status` ∈ `multiplied` / `not_multiplied` / `undetermined` and `scope` the same query-scope label the `tables` entries carry. `multiplied` means a join multiplied the rows behind THAT number, and `joins` names the join — whether that is wrong depends on what was asked, because the same total is right for line-item exposure and wrong for order revenue. `undetermined` means the check could not resolve what the aggregate reads (a `COUNT(*)`, an unqualified column, a CTE the walk does not enter), so do **not** report it as clean. `findings` is a different question — whether the arithmetic is meaningful at all — and a number can be un-multiplied and still meaningless |
...
**Read `tables.items[].filters` before you write the answer.** Nothing applies a declared filter to your statement, so this is where you find out whether yours honored the ones the org declared. A filter whose `status` is `omitted` means the answer counted rows the org's own definition of that table excludes — say so in the answer, and offer to re-run with it written in. `undetermined` means the check could not settle it (a `HAVING`, an outer-join `ON`, a predicate the scope resolver could not read), so it is neither a pass nor a failure and is worth one sentence. The fact lives per reference, inside the `tables` section, and there is no flat statement-level key for it anywhere on the receipt: a filter satisfied inside a CTE body is not satisfied for the statement that reads that CTE, and one list for the whole statement cannot say which was which. The same holds across a set operation — a filter `applied` in one arm and `omitted` in another is a real, reportable asymmetry, and the arm ordinal on `scope` is how you name which arm dropped it. `model_version` comes from the newest `.snapshots/<hash>/` dir (null on a pre-trust-layer model — renders gracefully).
...
That is exactly what the report's "metrics you haven't approved" banner reads, so an entry written any other way silently never appears. **Don't flag trivial primitives** (a bare `COUNT(*)` / `SUM(amount)`). For a multi-section report, run `sm receipt` per section's SQL and merge each section's `items` (dedup tables by `ref`+`qname`, metrics by name, joins by `from_to`+`scope`+`predicate`+`status` — a join has no name to dedup on, and `status` is load-bearing in that key because a join that wrote no predicate of its own has a null one: an explicit `CROSS JOIN` and a comma join over the same two tables in the same scope are otherwise the same key, and they are different facts) while KEEPING each section's `undetermined` — or pass the dominant query's SQL.

For a 1×1 scalar answer with no chart (Phase 4e skips the report), still run `sm receipt`, and surface inline in the chat answer every join in `joins.items` that HAS a `review_state` and it is `!= "approved"` ("Note: this used 1 unreviewed join, orders → customers") — a `null` one is a join nothing matched to a declaration, which is on the section's `undetermined` marker rather than a sign-off warning. Same for any `columns.items` entry with `kind: "output"` and `status: "matched"` whose `review_state` isn't approved — and name the output column it computes, which the entry now carries. An entry whose status is `unmatched` is the opposite signal and worth saying too: every declared binding was read and this number is not the org's agreed definition.
