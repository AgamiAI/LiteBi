...
It returns JSON: `{action, risk, sql, units, reason}` (on a refuse it instead returns `{action, risk, reason, suggestion, sql}` with exit 1). The returned `sql` is always the statement you handed in, byte for byte — this command never rewrites, so it either clears your statement or refuses it. (`units` is the `{output_column: unit}` map traced through that statement — keep it for the table render in 4d.)
- `action: "refuse"` (a fan/chasm trap: a join multiplies the rows an aggregate is computed from) → **don't execute, and don't silently guess.** Whether that multiplication is a *bug* depends on what was asked — a total multiplied by a join is wrong for order revenue and right for line-item exposure — and the pre-flight does not have the question. You do. So the judgement is yours to make here, out loud, rather than the guard's to make silently. Read `reason` + `suggestion`, then branch on whether a faithful rebuild is unambiguous:
  - **Single faithful rebuild** — there's one restructuring that preserves exactly what the user asked for (typically a chasm trap: pre-aggregate each measure in its own CTE, then outer-join; or move the aggregate into a window function to keep raw rows; or, where the many side is touched nowhere but the `ON` clause, drop the redundant join). Regenerate the SQL per `suggestion`, re-prepare, and **say in the answer** that the query was restructured to avoid a `<risk>` (nothing else will say it: the restructured statement no longer trips the check, so the receipt assembled from it carries no verdict, and the panel draws none — the sentence is yours to write). The user gets the right number *with* provenance — not a silent swap. That sentence is the whole difference between this and the guard rewriting the statement underneath the user, which it does not do.
...
- `action: "allow"` → use the returned `sql`; it is your statement verbatim. Nothing was AND-ed into it and nothing was dropped from it, so do **not** claim any declared filter on the receipt — if the question needed one, you wrote it in yourself at step 6.
