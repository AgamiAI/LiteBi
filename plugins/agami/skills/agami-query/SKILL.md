...
2. **Schema context** — the `get_table_context` output for the chosen tables (columns + types + caveats + value_transforms), the area's relationships (rendered as `from.col → to.col [cardinality]`), and the area's metrics (`<name>: <binding> -- <calculation>` + synonyms). **`default_filters` ARE yours to apply.** Nothing applies them for you: the executor stopped injecting them (ACE-042) and does not yet report them (ACE-099). If a table declares one and the question does not deliberately ask about the rows it excludes, write it into the `WHERE` clause, substituting the alias you used for `{alias}`. Also DO honor any caveats.
