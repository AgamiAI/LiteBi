...
def test_no_exemption_is_stale():
    ...
    assert not (set(_EXEMPT) - live), (
        f"exemptions no longer matching any parse: {set(_EXEMPT) - live}"
    )
...
def test_the_guard_battery_parses_in_exactly_one_place():
    ...
    runtime_parses = [(r, o) for r, o, _ in ALL_CALLS if r.endswith("semantic_model/runtime.py")]
