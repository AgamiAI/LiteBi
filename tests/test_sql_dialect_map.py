...
def _org(*storage_types: str) -> m.Datasource:
    ...
    return m.Datasource(
        ...
        version=1,
        storage_connections=[
            m.StorageConnection(name=f"c{i}", storage_type=st) for i, st in enumerate(storage_types)
        ],
    )
...
def test_each_storage_type_resolves_and_parses(storage_type):
    ...
    tree = sqlglot.parse_one("SELECT a FROM t", dialect=dialect, error_level=ErrorLevel.RAISE)
...
def test_single_connection_resolves(storage_type):
    assert sd.resolve_datasource_dialect(_org(storage_type)) == sd.sqlglot_dialect(storage_type)
