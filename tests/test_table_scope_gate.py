...
The gate returns `guardrail.Refusal | None`: `None` is "satisfied", a `Refusal` is
a stop. There is no allow object, because an allow has nothing to grade.
"""
...
def _assert_refuses(refusal, *tables: str) -> None:
    """The refusal names exactly `tables` and nothing else.

    Replaces the old `.offending_tables == [...]` + `"x" in .reason` pair: the offending names
    are no longer a separate field, they are echoed into `detail`. Asserted by exact equality
    rather than substring, which is strictly stronger — a refusal that had also listed the tables
    the model DOES declare would still satisfy an `in` check, and that is the disclosure the
    guardrail contract forbids.
    """
    ...
    assert refusal.remediation == ("Add the table to the model (agami-connect / '/agami-model'), "
...
def test_undeclared_table_refused():
    _assert_refuses(rt.check_table_scope("SELECT * FROM sqlite_master", _scope_org()),
                    "sqlite_master")
...
def test_join_with_undeclared_refused_lists_only_bad_one():
    _assert_refuses(rt.check_table_scope(
        "SELECT * FROM orders o JOIN payments p ON p.order_id = o.id", _scope_org()), "payments")
...
def test_cte_body_referencing_undeclared_refused():
    _assert_refuses(rt.check_table_scope(
        "WITH t AS (SELECT * FROM secret_table) SELECT * FROM t", _scope_org()), "secret_table")


def test_subquery_alias_allowed():
    # derived-table alias `x` is not a table; the inner `orders` is declared.
    assert rt.check_table_scope("SELECT * FROM (SELECT id FROM orders) x", _scope_org()) is None
...
def test_set_operation_arm_scoped():
    ...
    _assert_refuses(rt.check_table_scope(
        "SELECT id FROM orders UNION SELECT id FROM secret_table", _scope_org()), "secret_table")
